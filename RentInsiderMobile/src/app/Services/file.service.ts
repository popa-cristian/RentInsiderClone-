...
export class FileUploadService {
 ...
 invokeFirstComponentFunction = new EventEmitter();    
  ...
  pushFileToStorage(fileUpload: Myfile,Property:Property): Observable<number | undefined> {
  ...
  pushFileToStorage_Renter(fileUpload: Myfile,Property:Property): Observable<number | undefined> {
  ...
  getFiles(renterid:string){
      return this.afs.collection("Files",ref=>ref.where('renterID','==',renterid)).snapshotChanges().pipe
      (map(changes => {
  ...
  getFilesBelongingtoOwner(ownerID:string){
    return this.afs.collection("Files",ref=>ref.where('ownerID','==',ownerID)).snapshotChanges().pipe
    (map(changes => {
      return changes.map(a => {
        const data = a.payload.doc.data() as Myfile;
        data.id = a.payload.doc.id;
  ...
  getFilesOfRenter(ownerID:string,renterID:string){
    return this.afs.collection("Files",ref=>ref.where('ownerID','==',ownerID).where('renterID','==',renterID)).snapshotChanges().pipe
    (map(changes => {
      return changes.map(a => {
        const data = a.payload.doc.data() as Myfile;
        data.id = a.payload.doc.id;
  ...
  async getFilebyID(id:string){
    return await this.afs.collection('Files').doc(id).ref.get().then(doc=>{
      if(doc.exists)
      {
        return doc.data() as Myfile;
      }
      else
        throw new Error("Fisierul nu exista!");
  });    
  }
  
  private deleteFileDatabase(fileupload:Myfile) {
        this.afs.collection("Files",ref=>ref.where('url','==',fileupload.url)).snapshotChanges().pipe
        (map(changes => {
        changes.map(a => {
          const data = a.payload.doc.data() as Myfile;
          data.id = a.payload.doc.id;
  ...
  private deleteFileStorage(file:Myfile): void {
    ...
    storageRef.child(file.ownerID).child(file.renterID).child(file.name).delete();
  }
  response(){
  ...
  test(){
    this.afs.collection("Files").doc("Merge").set({
      merge:"merge"
    })
  }
