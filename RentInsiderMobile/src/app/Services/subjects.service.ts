...
export class SubjectsService{
    ...
    addSubject(subject:Subject){
      return this.afs.collection("Subjects").add({
    ...
    updateSubject(subject:Subject){
      return this.afs.doc(`Subjects/${subject.id}`).update({
        title:subject.title,
        description:subject.description,
        importance:subject.importance,
      });
    ...
    getPropertyBySubjectID(ID:string){
      return this.afs.collection('Properties').doc(ID).ref.get().then(doc=>{
        if(doc.exists)
        {
    ...
    getSubjectByID(ID:string){
      return this.afs.collection('Subjects').doc(ID).ref.get().then(doc=>{
        if(doc.exists) {
    ...
    getSubjectsCreatedOnProperty(property:Property){
      return this.afs.collection("Subjects", subject => subject.where('propertyID', '==', property.id)).snapshotChanges().pipe
    (map(changes => {
    ...
    getSubjects() {
        return this.database.collection('Subjects').snapshotChanges().pipe
        (map(changes => {
          return changes.map(a => {
      ...
      test(){
        this.afs.collection("Subjects").doc("Merge").set({
          merge:"merge"
        })
      }
}