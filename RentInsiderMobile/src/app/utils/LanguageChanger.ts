...
export function ChangePageLanguage(languageWanted: string, languageUsed: string, translateService1: TranslateService, translateService2: TranslateService): void;

export function ChangePageLanguage(languageWanted: string, languageUsed: string, ...args: any[]): void {
      if (languageUsed != ApplicationLanguages[languageWanted]) {
            if(args.length > 0)
            {
                  args[0].use(ApplicationLanguages[languageWanted]);
                  if(args.length == 2)
                  args[1].use(ApplicationLanguages[languageWanted]);
            }
            else
            {
                  return;
