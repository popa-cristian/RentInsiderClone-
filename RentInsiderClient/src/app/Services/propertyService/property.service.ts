...
import { SignInServiceService } from '../signInService/sign-in-service.service';
...
export class PropertyService {
  ...
  getPropertyByID(propertyID: string) {
    return this.afs
      ...
      .then((doc) => {
        ...
        } else console.log('Nu exista proprietatea selectata');
      });
  }
  ...
  getPropertiesFromUser() {
    ...
    return this.afs
      .collection('Properties', (property) =>
        property.where('ownerID', '==', ownerId)
      )
      .snapshotChanges()
      .pipe(
        map((changes) => {
  ...
  getMockProperties() {
    var values = this.http.get<Property[]>('../../assets/properties.json');
    console.log(values);
    return values;
  }
}
